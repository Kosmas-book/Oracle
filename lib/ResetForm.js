...
export default function ResetForm({ token }) {
  ...
  async function submit() {
    ...
    else setErr(data.error || "Κάτι πήγε στραβά.");
  }
      ...
      {!token ? (
        ...
        <>
          ...
          <input
            type="password"
            inputMode="numeric"
            placeholder="Νέο PIN (6–12 ψηφία)"
            value={pin}
          ...
          <input
            ...
            onKeyDown={(event) => event.key === "Enter" && pin && submit()}
          />
          {err && <p className="msg-err">{err}</p>}
          <button className="btn" onClick={submit} disabled={busy || !/^\d{6,12}$/.test(pin)}>
            Αποθήκευση PIN
          </button>
        </>
      )}
