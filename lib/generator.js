...
export function generateWeek({
  ...
  // 1C: ΔΕΝ απαιτείται Ρ το Σάββατο πριν από το night block. Ο εργαζόμενος
  // μπορεί να δουλέψει κανονικά το Σάββατο, αρκεί να τηρείται το 11ωρο πριν
  // από την έναρξη του Β της Κυριακής (22:00).
  if (
    hasNight &&
    nextNightPersonId &&
    ...
    if (satCode && !restOk(satCode, "Β")) {
      nightConflicts.push(
        `Ο επόμενος βραδινός (${byId[nextNightPersonId].name}) έχει ${satCode} το Σάββατο — δεν μεσολαβούν ${MIN_REST_HOURS} ώρες μέχρι την έναρξη του Β της Κυριακής (22:00).`
      );
    }
    if (!grid[nextNightPersonId][6]) {
      ...
      // 1F: ο planned δεν μπορεί — ψάχνουμε αντικαταστάτη· ο actual κρατιέται.
      const sub = active
        .filter(
          (e) =>
  ...
  for (const e of active) {
    ...
    // ΠΡΟΤΙΜΗΣΗ: ο επόμενος βραδινός ξεκουράζεται το Σάββατο πριν μπει σε
    // επτά νύχτες. ΔΕΝ είναι επιπλέον ρεπό — είναι το κανονικό του, απλώς
    // τοποθετημένο εκεί. Αν δεν του περισσεύει ρεπό (π.χ. μόλις τελείωσε δικό
    // του μπλοκ και πήρε ήδη Ρ Δευτέρας), δουλεύει κανονικά το Σάββατο.
    if (
      hasNight &&
      e.id === nextNightPersonId &&
      restNeeded > 0 &&
  ...
  for (const pt of active.filter(isPart)) {
    ...
    if (workdays[pt.id] < ptTarget) {
      warnings.push(
        `${pt.name} (part-time): ${workdays[pt.id]} μέρες αντί για ${
          wtPt != null && wtPt !== "" ? "στόχο " + ptTarget : "ελάχιστο " + ptTarget
        }.`
      );
    }
  }
  ...
  for (const e of active) {
    const fillRest = !isPart(e) || targetFor(e).exact != null;
    for (let d = 0; d < 7; d++) {
      if (grid[e.id][d] === "" && fillRest) grid[e.id][d] = "Ρ";
    }
  }
  ...
  if (hasNight && nextNightPersonId && byId[nextNightPersonId]) {
    ...
    // Ενημερωτικά (ΟΧΙ σφάλμα): κανονικά ο επόμενος βραδινός ξεκουράζεται το
    // Σάββατο. Όταν δεν του περισσεύει ρεπό, δουλεύει — και το λέμε ρητά.
    if (row[5] && row[5] !== "Ρ" && row[5] !== "Ο")
      warnings.push(
        `${byId[nextNightPersonId].name}: δουλεύει ${row[5]} το Σάββατο πριν μπει σε επτά νύχτες — δεν του περίσσευε ρεπό αυτή την εβδομάδα.`
      );
